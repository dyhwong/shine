...
var width = 4/3*(window.innerHeight-100);
var height = window.innerHeight-100;
var game_playing; 


audio_Demo = new Audio("playlist/demo.mp3");
...
//calls helper functions and stuff	
	...
	var videoLoop = function(){
		...
		if (game_playing){
			...
			if (current_date - start_date >= song_length * 1000){
				game_playing = false;
				//$.post('/gamepage', {
...
function generatePoint(){
	// Generate a point 
	x_new = Math.floor(Math.random() * (width/10-5) + 3) * 10; /*(10<x<500)*/
	y_new = Math.floor(Math.random() * (height/10-5) + 3) * 10; /*(10<x<350)*/
